...
class NaturalCuriosity : ItemBase("organic_curiosity") {
    ...
    override fun onItemRightClick(worldIn: World, playerIn: EntityPlayer, handIn: EnumHand): ActionResult<ItemStack> {
        if (!worldIn.isRemote) {
            ...
            when(worldIn.rand.nextInt(5)){
                ...
                2->{
                    ThaumcraftApi.internalMethods.addKnowledge(playerIn, IPlayerKnowledge.EnumKnowledgeType.OBSERVATION, ResearchCategories.getResearchCategory("BIOMANCY"), MathHelper.getInt(playerIn.rng, oProg / 2, oProg))
                    ThaumcraftApi.internalMethods.addKnowledge(playerIn, IPlayerKnowledge.EnumKnowledgeType.THEORY, ResearchCategories.getResearchCategory("BIOMANCY"), MathHelper.getInt(playerIn.rng, tProg / 3, tProg / 2))
                }
                else->{
                    ThaumcraftApi.internalMethods.addKnowledge(playerIn, IPlayerKnowledge.EnumKnowledgeType.THEORY, ResearchCategories.getResearchCategory("BIOMANCY"), MathHelper.getInt(playerIn.rng, tProg / 3, tProg / 2))
                }
