...
class CardDissect : TheorycraftCard() {
    ...
    override fun getLocalizedName(): String {
        return TextComponentTranslation("card.thaumic_arcana.study_flesh.name", TextFormatting.DARK_BLUE.toString() + this.itemStack.displayName + TextFormatting.RESET + "" + TextFormatting.BOLD).formattedText
    }

    override fun getLocalizedText(): String {

        return TextComponentTranslation("card.thaumic_arcana.study_flesh.text", TextFormatting.DARK_BLUE.toString() + this.itemStack.displayName + TextFormatting.RESET).formattedText
    }

    override fun activate(entityPlayer: EntityPlayer, researchTableData: ResearchTableData): Boolean {
        researchTableData.addTotal(this.researchCategory, 15)
        return true
    }
